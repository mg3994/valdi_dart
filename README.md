...
14. **Content Windowing & Native Scroll Telemetry**:
...
28. **Flutter Layout & Core Vocabulary**:
...
31. **Social Authentication & Credential Sheets**:
...
32. **Camera & Lottie Sticker Engines**:
...
33. **Search Bar Choreography & Navigation**:
...
34. **Animations & Shared Element Transitions**:
...
41. **Dual-Mode Rendering Pipeline (Native Views & Optional Skia Canvas)**:
...
void main() async {
  // Video Texture Streamer
  final texture = VideoTextureStreamer();
  final texId = await texture.registerTexture('https://cdn.example.com/video.mp4');

  // Event Stream Socket
  final stream = ValdiEventStream<String>('sensors');
  stream.stream.listen((ev) => print('Sensor: $ev'));

  final root = Wrap(
    direction: FlexDirection.row,
    children: [
      Text('Texture ID: $texId'),
